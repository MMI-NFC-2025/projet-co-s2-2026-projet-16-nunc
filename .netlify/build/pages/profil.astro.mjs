...
Retour
</a> <form method="post"> <input type="hidden" name="action" value="logout"> <button type="submit" aria-label="Se déconnecter" class="flex h-12 w-12 items-center justify-center rounded-full bg-white-bk/50"> ${renderComponent($$result2, "Image", $$Image, { "src": Power, "alt": "", "class": "h-7 w-7" })} </button> </form> </div> <div class="mt-4 flex flex-col items-center"> <form id="avatar-form" method="post" enctype="multipart/form-data" class="relative mt-4"> <input type="hidden" name="action" value="avatar"> ${renderComponent($$result2, "UserAvatar", $$UserAvatar, { "user": user, "pb": pb, "size": "h-40 w-40" })} <input id="avatar-input" name="avatar" type="file" accept="image/*" class="hidden"> <label for="avatar-input" aria-label="Modifier la photo de profil" class="absolute top-1 right-1 flex h-12 w-12 cursor-pointer items-center justify-center rounded-full bg-white-c"> ${renderComponent($$result2, "Image", $$Image, { "src": Edit, "alt": "", "class": "h-6 w-6" })} </label> </form> <h3 id="username" class="mt-5 max-w-full break-words px-4 text-center text-white-bk">${user.username}</h3> <div id="premium-badge"${addAttribute([
...
Premium
</div> <small id="date-inscription" class="mt-2 text-white-c">Membre depuis ${dateInscription}</small> </div> <div class="mt-7 rounded-3xl bg-white-c p-6"> <div class="flex items-center gap-4"> <div class="w-10 h-10 rounded-full bg-beige-c flex items-center justify-center"> ${renderComponent($$result2, "Image", $$Image, { "src": Trophy, "alt": "", "class": "w-6 h-6" })} </div> <div> <h4 id="niveau" class="font-titan leading-none">Niveau ${niveau}</h4> <small id="points" class="mt-1 block text-marron-c">${points} / ${maxXp} points</small> </div> </div> <div class="mt-8 h-5 bg-white rounded-full overflow-hidden"> <div id="progress-bar" class="h-full bg-orange-c rounded-full"${addAttribute(`width:${progress}%`, "style")}></div> </div> </div> </section> <div class="mt-8 grid grid-cols-3 gap-4 px-6"> <article class="rounded-2xl border border-beige-c bg-white-c p-5 text-center"> ${renderComponent($$result2, "Calendrier", Calendrier, { "class": "mx-auto h-7 w-7 text-paprika-c" })} <h4 id="nb-sorties" class="mt-4 font-titan">${nbSorties}</h4> <small class="mt-2 block text-marron-c">Sorties</small> </article> <article class="rounded-2xl border border-beige-c bg-white-c p-5 text-center"> ${renderComponent($$result2, "Trophee", Trophy, { "class": "mx-auto h-7 w-7 text-paprika-c" })} <h4 id="nb-organisations" class="mt-4 font-titan">${nbOrganisations}</h4> <small class="mt-2 block text-marron-c">Organiser</small> </article> <article class="rounded-2xl border border-beige-c bg-white-c p-5 text-center"> ${renderComponent($$result2, "Amis", Amis, { "class": "mx-auto h-7 w-7 text-paprika-c" })} <h4 id="nb-amis" class="mt-4 font-titan">${nbAmis}</h4> <small class="mt-2 block text-marron-c">Ami(e)s</small> </article> </div> <section class="mt-10 px-6"> <div class="flex items-center justify-between"> <h3 class="text-paprika-c">Mes cadres</h3> <small class="text-marron-c">${cadresInventaire.length} possédé${cadresInventaire.length > 1 ? "s" : ""}</small> </div> ${cadresInventaire.length > 0 ? renderTemplate`<div class="mt-8 grid grid-cols-2 gap-6"> ${cadresInventaire.map((item) => renderTemplate`<form method="post"> <input type="hidden" name="action" value="equip-cadre"> <input type="hidden" name="inventaireId"${addAttribute(item.id, "value")}> <button${addAttribute([
    "relative w-full rounded-xl border bg-white-c p-4 text-center",
    item.equipe ? "border-3 border-paprika-c" : "border border-mocha-c"
  ], "class:list")}> ${item.equipe && renderTemplate`<span class="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full bg-paprika-c text-white-bk">
✓
</span>`} <img${addAttribute(getFileUrl(item.cadre, item.cadre.image, pb), "src")}${addAttribute(`Cadre ${item.cadre.nom}`, "alt")} loading="lazy" class="mx-auto h-40 w-40 object-contain"> <p class="mt-4">${item.cadre.nom}</p> <small class="mt-1 block text-marron-c"> ${item.type === "premium" ? "Cadre premium" : "Cadre classique"} </small> </button> </form>`)} </div>` : renderTemplate`<article class="mt-5 rounded-2xl border border-beige-c bg-white-c px-5 py-6 text-center"> <p class="font-bold text-bordeaux-c">Aucun cadre pour le moment</p> <small class="mt-2 block text-marron-c">Les cadres achetés dans la boutique apparaîtront ici.</small> </article>`} </section> <nav class="mt-12 px-6 text-center"> <div class="flex flex-wrap items-center justify-center gap-3 text-basesur la "> <a href="/a-propos">À propos</a> <span>•</span> <a href="/mentions-legales">Mentions légales</a> <span>•</span> <a href="/contact">Contact</a> </div> </nav> </main> ` })} ${renderScript($$result, "/Users/Lina/Documents/GitHub/projet-co-s2-2026-projet-16-nunc/src/pages/profil.astro?astro&type=script&index=0&lang.ts")}`;

...
En attente
</a> </div> <div id="friends-list" class="mt-8 flex flex-col gap-4"> ${search.length >= 1 ? searchResults.length > 0 ? searchResults.map((person) => renderTemplate`<article class="flex w-full items-center gap-3 rounded-2xl border-2 border-mocha-c bg-white-c p-3"> <a${addAttribute(`/amis/${person.id}`, "href")} class="flex min-w-0 flex-1 items-center gap-3"> ${renderComponent($$result2, "UserAvatar", $$UserAvatar, { "user": person, "pb": pb, "size": "h-16 w-16" })} <div class="min-w-0"> <h4 class="break-words font-bold text-bordeaux-c">${person.username}</h4> <small class="text-marron-c">Utilisateur</small> </div> </a> <form method="post" class="shrink-0"> <input type="hidden" name="action" value="add"> <input type="hidden" name="friendId"${addAttribute(person.id, "value")}> <button class="rounded-full bg-paprika-c px-4 py-2 text-sm text-white">Ajouter</button> </form> </article>`) : renderTemplate`<p class="rounded-2xl bg-white-c p-5 text-marron-c">Aucun utilisateur trouvé.</p>` : activeTab === "requests" ? requests.length > 0 ? requests.map((request) => {
    const sender = request.expand?.utilisateur1;
    return renderTemplate`<article class="flex w-full items-center gap-3 rounded-2xl border-2 border-mocha-c bg-white-c p-3"> <a${addAttribute(`/amis/${sender?.id}`, "href")} class="flex min-w-0 flex-1 items-center gap-3"> ${renderComponent($$result2, "UserAvatar", $$UserAvatar, { "user": sender, "pb": pb, "size": "h-16 w-16" })} <div class="min-w-0"> <h4 class="break-words font-bold text-bordeaux-c">${sender?.username || "Utilisateur"}</h4> <small class="text-marron-c">Demande reçue</small> </div> </a> <div class="flex shrink-0 flex-col gap-2"> <form method="post"> <input type="hidden" name="action" value="accept"> <input type="hidden" name="friendshipId"${addAttribute(request.id, "value")}> <button class="rounded-full bg-paprika-c px-4 py-2 text-sm text-white">Accepter</button> </form> <form method="post"> <input type="hidden" name="action" value="refuse"> <input type="hidden" name="friendshipId"${addAttribute(request.id, "value")}> <button class="rounded-full border-2 border-mocha-c px-4 py-2 text-sm text-mocha-c">Refuser</button> </form> </div> </article>`;
    ...
    return renderTemplate`<article class="flex w-full items-center gap-3 rounded-2xl border-2 border-mocha-c bg-white-c p-3"> <a${addAttribute(`/amis/${receiver?.id}`, "href")} class="flex min-w-0 flex-1 items-center gap-3"> ${renderComponent($$result2, "UserAvatar", $$UserAvatar, { "user": receiver, "pb": pb, "size": "h-16 w-16" })} <div class="min-w-0"> <h4 class="break-words font-bold text-bordeaux-c">${receiver?.username || "Utilisateur"}</h4> <small class="text-marron-c">Demande envoyée</small> </div> </a> <span class="shrink-0 whitespace-nowrap rounded-full bg-white-bk px-4 py-2 text-sm text-marron-c">En attente</span> </article>`;
  }) : renderTemplate`<p class="rounded-2xl bg-white-c p-5 text-marron-c">Aucune demande en attente.</p>` : currentFriends.length > 0 ? currentFriends.map((friend) => renderTemplate`<article class="flex w-full items-center gap-3 rounded-2xl border-2 border-mocha-c bg-white-c p-3"> <a${addAttribute(`/amis/${friend.id}`, "href")} class="flex min-w-0 flex-1 items-center gap-3"> ${renderComponent($$result2, "UserAvatar", $$UserAvatar, { "user": friend, "pb": pb, "size": "h-16 w-16" })} <div class="min-w-0"> <h4 class="break-words font-bold text-bordeaux-c">${friend.username}</h4> <small class="text-marron-c">Ami(e)</small> </div> </a> <span class="shrink-0 whitespace-nowrap rounded-full bg-paprika-c px-4 py-2 text-sm text-white">Ami</span> </article>`) : renderTemplate`<p class="rounded-2xl bg-white-c p-5 text-marron-c">Tu n'as pas encore d'ami(e).</p>`} </div> </main> ` })} ${renderScript($$result, "/Users/Lina/Documents/GitHub/projet-co-s2-2026-projet-16-nunc/src/pages/amis/index.astro?astro&type=script&index=0&lang.ts")}`;

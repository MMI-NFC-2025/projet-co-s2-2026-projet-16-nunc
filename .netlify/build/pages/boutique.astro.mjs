...
Premium
    ...
    return renderTemplate`<article class="rounded-xl border border-mocha-c bg-white-c p-4 text-center"> <img class="cadre-image mx-auto h-40 w-40 object-contain"${addAttribute(getFileUrl(cadre, cadre.image, pb), "src")} alt=""> <p class="cadre-nom mt-4">${cadre.nom}</p> <h3 class="cadre-prix mt-2 text-paprika-c">${cadre.prix} pts</h3> ${possede ? renderTemplate`<button disabled class="cadre-bouton mt-4 w-full rounded-full bg-beige-c px-6 py-3 text-white-bk">Possédé</button>` : renderTemplate`<form method="post"> <input type="hidden" name="action" value="acheter-cadre"> <input type="hidden" name="cadreId"${addAttribute(cadre.id, "value")}> <button class="cadre-bouton mt-4 w-full rounded-full bg-paprika-c px-6 py-3 text-white-bk">Acheter</button> </form>`} </article>`;

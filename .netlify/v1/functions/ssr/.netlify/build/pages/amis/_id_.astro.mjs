...
import { g as getUserById, a as getNbSortiesParticipees, b as getNbSortiesOrganisees, c as getNbAmis, d as getFileUrl } from '../../chunks/backend_CmHEyYCM.mjs';
...
const $$id = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const nbSorties = await getNbSortiesParticipees(id);
  const nbOrganisations = await getNbSortiesOrganisees(id);
  const nbAmis = await getNbAmis(id);
...
Retour
</a> <div class="mt-4 flex flex-col items-center"> <img${addAttribute(getFileUrl(user, user.avatar), "src")} alt="" class="h-40 w-40 rounded-full object-cover"> <h3 class="mt-5 text-center text-white-bk">${user.username}</h3> ${user.premium && renderTemplate`<div class="mt-3 flex items-center gap-2 rounded-full bg-white-c/40 px-5 py-2 font-bold text-white-bk"> ${renderComponent($$result2, "Image", $$Image, { "src": Courone, "alt": "", "class": "h-4 w-4" })}
Premium

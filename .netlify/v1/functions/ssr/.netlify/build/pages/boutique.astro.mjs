...
import { l as loadAstroAuth, k as acheterCadre, g as getUserById, o as obtenirCadrePremium, m as getCadres, n as getCadresUtilisateur, p as getCadresPremium, q as getCadresExclusifs, t as getFileUrl } from '../chunks/backend_CNX6c8lU.mjs';
...
const $$Boutique = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "Boutique", "hideHeader": true }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<main class="pb-32"> ${message && renderTemplate`<div${addAttribute([
    ...
    "items-center gap-2 rounded-full bg-paprika-c px-4 py-2 text-white-bk whitespace-nowrap",
    user.premium ? "flex" : "hidden"
  ], "class:list")}> ${renderComponent($$result2, "Image", $$Image, { "src": Courone, "alt": "", "class": "h-4 w-4" })}
Premium
</div> </div> <div class="mt-6 h-5 bg-white rounded-full overflow-hidden"> <div id="progress-bar" class="h-full bg-orange-c rounded-full"${addAttribute(`width:${progress}%`, "style")}></div> </div> </div> </section> <section class="px-8"> <a href="/premium" class="mt-8 flex items-center justify-between gap-4 rounded-3xl bg-linear-to-br from-paprika-c to-yellow-c px-7 py-7"> <div class="min-w-0"> <div class="flex items-center gap-4"> ${renderComponent($$result2, "Courone", Courone, { "class": "h-7 w-7 shrink-0" })} <h3 class="whitespace-nowrap text-white-bk">Pass Premium</h3> </div> <small class="text-white-bk">Profite de réductions exclusives et d'avantages illimités</small> </div> ${renderComponent($$result2, "Fleche", Fleche, { "class": "h-14 w-14 shrink-0" })} </a> <h3 class="text-center text-paprika-c mt-7">Déco Classique</h3> <div id="cadres-list" class="mt-8 grid grid-cols-2 gap-6"> ${cadres.map((cadre) => {
    const possede = cadresPossedes.has(cadre.id);
    const prixCadre = user.premium ? Math.ceil(cadre.prix * 0.8) : cadre.prix;
    return renderTemplate`<article class="rounded-xl border border-mocha-c bg-white-c p-4 text-center"> <img class="cadre-image mx-auto h-40 w-40 object-contain"${addAttribute(getFileUrl(cadre, cadre.image, pb), "src")}${addAttribute(`Cadre ${cadre.nom}`, "alt")} loading="lazy"> <p class="cadre-nom mt-4">${cadre.nom}</p> <h3 class="cadre-prix mt-2 text-paprika-c">${prixCadre} pts</h3> ${user.premium && renderTemplate`<small class="text-marron-c">au lieu de ${cadre.prix} pts</small>`} ${possede ? renderTemplate`<button disabled class="cadre-bouton mt-4 w-full rounded-full bg-beige-c px-6 py-3 text-white-bk">Possédé</button>` : renderTemplate`<form method="post"> <input type="hidden" name="action" value="acheter-cadre"> <input type="hidden" name="cadreId"${addAttribute(cadre.id, "value")}> <button class="cadre-bouton mt-4 w-full rounded-full bg-paprika-c px-6 py-3 text-white-bk">Acheter</button> </form>`} </article>`;

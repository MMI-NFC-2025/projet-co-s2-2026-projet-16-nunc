import { createSortie, refreshCurrentUser, addUserPoints } from '../../backend/backend.mjs';
...
const classicBtn =
    document.getElementById('classic-btn');

const crawlBtn =
    document.getElementById('crawl-btn');

const typeInput =
    document.getElementById('type-sortie');

const crawlBars =
    document.getElementById('crawl-bars');
...
classicBtn.addEventListener('click', (e) => {
    ...
    classicBtn.classList.remove(
        'bg-white-bk',
        'text-yellow-c',
        'border-yellow-c'
    );

    classicBtn.classList.add(
        'bg-paprika-c',
        'text-white',
        'border-paprika-c'
    );

    crawlBtn.classList.remove(
        'bg-paprika-c',
        'text-white',
        'border-paprika-c'
    );

    crawlBtn.classList.add(
        'bg-white-bk',
        'text-yellow-c',
        'border-yellow-c'
    );
...
form.addEventListener('submit', async (e) => {
    ...
    if (!user) {
        ...
        errorMessage.textContent =
            'Vous devez être connecté pour créer une sortie.';
    ...
    if (
        ...
        errorMessage.textContent =
            'Sans abonnement Premium, la limite est de 20 participants.';
    ...
    if (result.success) {
        ...
        errorMessage.textContent =
            'Une erreur est survenue lors de la création de la sortie.';
